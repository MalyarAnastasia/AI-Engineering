...
**Примечание:** Все критерии выполнены. Сервис запускается и работает корректно, модель обучена на реальных данных (4.5M+ записей) и используется в /predict endpoint, ноутбуки с экспериментами созданы, структура кода внятная, Dockerfile присутствует, конфиги и секреты управляются корректно, логи реализованы, выбор модели обоснован в report.md.

**Метрики модели (на реальных данных):**
- MAE = 498,000 руб
- RMSE = 704,000 руб
- R² = 0.77
- Обучено на 4,555,826 записях
