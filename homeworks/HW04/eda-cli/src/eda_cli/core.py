...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame, high_cardinality_threshold: int = 50, zero_share_threshold: float = 0.9,) -> Dict[str, Any]:
    """
    ...
    constant_cols_count = sum(col.unique <= 1 for col in summary.columns if col.non_null > 0)
    ...
    flags["has_high_cardinality_categoricals"] = high_card_cols_count > 0
    
    many_zero_cols_count = 0
    for name in df.columns:
        s = df[name]
        if ptypes.is_numeric_dtype(s) and s.count() > 0:
            zero_count = (s == 0).sum()
            zero_share = zero_count / s.count()
            if zero_share > zero_share_threshold:
                many_zero_cols_count += 1
                
    flags["has_many_zero_values"] = many_zero_cols_count > 0
